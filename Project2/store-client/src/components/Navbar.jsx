...
const Navbar = () => {
  const [showLogin, setShowLogin] = useState(false); // Declare the state to control login modal visibility
  const emailRef = useRef(null); // Initialize emailRef
  const passwordRef = useRef(null); // Initialize passwordRef
          ...
          <div className="w-full h-full flex flex-row justify-end items-center gap-8 font-bold">
            {Linksdata.map((link, index) => (
              <NavLink
                to={link.path}
                key={index}
                className="h-[65%] w-20 hover:bg-green-500 hover:text-white flex justify-center items-center rounded-sm"
              >
                {link.title}
              </NavLink>
            ))}
          ...
          <div className="h-1/2 w-1/3 flex flex-col justify-center items-center bg-white shadow-2xl rounded-md">
            <div className="h-full w-full flex flex-col justify-center items-center text-lg font-semibold">
              <div className="w-[80%] flex flex-row justify-between items-center">
                <h1 className="w-1/2 text-left my-6 font-bold text-purple-500">
                  Login
                </h1>
                <div
              ...
              <form
                className="h-[80%] w-[80%] flex flex-col justify-center items-center gap-8"
              ...
              >
                ...
                <button
                  type="submit"
                  className="w-full h-[3rem] shadow-sm bg-purple-500 text-white rounded-sm outline-none"
                >
                  Login
                </button>
              </form>
